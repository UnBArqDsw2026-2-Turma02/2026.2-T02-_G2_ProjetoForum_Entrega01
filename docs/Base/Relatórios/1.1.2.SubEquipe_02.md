...
A metodologia do Foco_01 combinou a elaboração de Rich Pictures complementares e a aplicação do NFR Framework para estruturar dois SIGs/NFR: um voltado à Segurança da Informação e outro complementar, voltado à Usabilidade. Como insumo, foram utilizados os achados da [Engenharia Reversa do Stack Overflow (Foco_02)](#processo-de-engenharia-reversa-aplicado). O trabalho foi organizado por responsabilidades principais — Rich Pictures, SIG/NFR e consolidação — seguido de revisão e integração do relatório.

Os Rich Pictures foram utilizados como modelagens visuais e pouco formais, com desenhos e textos curtos para representar perspectivas complementares de atores, objetivos, interações, relações e preocupações do subdomínio de Comunidade e Engajamento do `G2_ProjetoForum`. Os SIGs/NFR foram utilizados para organizar os requisitos não funcionais de Segurança da Informação e Usabilidade, seus subobjetivos, operacionalizações, contribuições e trade-offs. Em ambos os tipos de artefato, as observações do sistema de referência foram separadas das decisões de adaptação para o fórum proposto.
...
| --- | --- | --- |
| [#69 — Elaboração de Rich Picture complementar baseado na Engenharia Reversa](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G2_ProjetoForum_Entrega01/issues/69) | [036fb0a — inclusão do Rich Picture complementar](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G2_ProjetoForum_Entrega01/commit/036fb0a389a7bffa144fd613cf1e3fb1a24cecf7) | Elaboração do Rich Picture complementar sobre experiência de resolução e continuidade. |



### SIG/NFR — Segurança da Informação
...
<p align="center"><b>SIG/NFR de Segurança da Informação</b> — prevenção do abuso de reputação por votação.<br>
<b>Autor:</b> Luis Guilherme</p>
...
### SIG/NFR complementar — Usabilidade

#### Escolha e delimitação do requisito não funcional

Com base nos achados consolidados da Engenharia Reversa, foi escolhido o
requisito não funcional **Usabilidade**, delimitado à **clareza e previsibilidade
do estado da pergunta e da resolução**. O recorte trata da capacidade de o
usuário compreender o que está acontecendo, interpretar os sinais da
plataforma e identificar quais caminhos ainda estão disponíveis.

Essa delimitação deriva principalmente dos achados `ER-C01` a `ER-C04`, que
registram a publicação da pergunta, a colaboração e avaliação das respostas, a
aceitação como resolução pessoal e os caminhos de espera, fechamento e possível
reabertura. O SIG complementar não repete o foco do SIG de Segurança da
Informação: ele organiza as necessidades de compreensão, feedback e
previsibilidade que atravessam a experiência do usuário.

Assim, no SIG de Segurança da Informação, a Usabilidade aparece como um efeito
de trade-off afetado por controles de proteção; neste SIG complementar, ela é
tratada como objetivo principal, detalhado em compreensão da interação,
visibilidade do estado, feedback e orientação.

As operacionalizações apresentadas a seguir são **propostas de adaptação para
o `G2_ProjetoForum`**. Elas foram derivadas dos comportamentos observados e das
regras consultadas no Stack Overflow, mas não afirmam que a plataforma de
referência implemente exatamente essas decisões de projeto.

#### Diagrama SIG

![Diagrama SIG/NFR complementar de Usabilidade](./assets/SIG_Usabilidade_Clareza_Resolucao.svg)
<p align="center"><b>SIG/NFR complementar de Usabilidade</b> — clareza e previsibilidade do estado da pergunta e da resolução.<br>
<b>Autor:</b> Leonardo de Aquino</p>

No diagrama, as nuvens representam softgoals e os retângulos representam
operacionalizações. O softgoal principal é decomposto pelo operador gráfico
`∧` (AND) em **Compreensão da interação**, **Visibilidade do estado** e
**Feedback e orientação**, tratados como dimensões complementares. As
contribuições das operacionalizações são indicadas pelos rótulos `++` e `+`,
conforme a legenda do NFR Framework.

#### Rastreabilidade, operacionalizações e trade-offs

| Elemento do SIG | Tipo e decisão | Rastreabilidade e efeito esperado |
| --- | --- | --- |
| Usabilidade — clareza e previsibilidade da resolução | Softgoal principal | `ER-C01` a `ER-C04`: o usuário precisa compreender a publicação, a colaboração, a avaliação, a resolução e a continuidade da pergunta. |
| Compreensão da interação | Subobjetivo | `ER-C02` e `ER-C03`: diferencia os papéis de resposta, comentário e voto e evita confundir aceitação com avaliação coletiva. |
| Visibilidade do estado | Subobjetivo | `ER-C03` e `ER-C04`: torna compreensíveis estados como aberta, sem resposta, resolvida e fechada, além dos caminhos de continuidade. |
| Feedback e orientação | Subobjetivo | `ER-C01` e `ER-C04`: orienta a publicação, comunica os efeitos das ações e informa próximos passos quando a resolução não é imediata. |
| Diferenciar resposta, comentário e voto | Operacionalização com `++ MAKE` | `ER-C02`: reduz ambiguidades entre produção de conteúdo, interação e avaliação comunitária. |
| Destacar aceitação e resolução pessoal | Operacionalização com `++ MAKE` | `ER-C03`: explicita que a aceitação é um sinal do autor e não equivale necessariamente à resposta mais votada. |
| Exibir estado da pergunta | Operacionalização com `++ MAKE` | `ER-C03` e `ER-C04`: permite compreender a situação atual da pergunta e o caminho disponível. |
| Indicar continuidade e possibilidade de reabertura | Operacionalização com `+ HELP` | `ER-C04`: torna previsíveis a espera, o fechamento e o retorno à colaboração. |
| Validar campos e orientar a publicação | Operacionalização com `+ HELP` | `ER-C01`: reduz incertezas na criação da pergunta e orienta o preenchimento inicial. |
| Informar resultado das ações e orientar os próximos passos | Operacionalização com `+ HELP` | `ER-C01` e `ER-C04`: fornece feedback compreensível após ações e em situações de ausência de resposta ou resolução. |

O principal trade-off é entre **clareza e fricção**: validações, distinções e
estados explícitos melhoram a previsibilidade, mas podem aumentar o esforço
inicial ou o custo cognitivo da participação. Por isso, o feedback deve
explicar o motivo das orientações e os próximos passos, evitando que os
mecanismos de usabilidade se tornem barreiras opacas.

No vocabulário utilizado no SIG, `HELP` indica contribuição positiva parcial,
`MAKE` indica contribuição positiva forte e `HURT` indicaria contribuição
negativa. A legenda textual explicita a interpretação das relações para que os
símbolos do diagrama não sejam confundidos com regras observadas diretamente
na plataforma de referência.

#### Rastreabilidade do SIG/NFR complementar

| Issue | Commit associado | Contribuição registrada |
| --- | --- | --- |
| [#70 — Elaboração de SIG/NFR de Usabilidade sobre clareza e previsibilidade da resolução](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G2_ProjetoForum_Entrega01/issues/70) | [294e0e0 — inclusão do SIG/NFR complementar de Usabilidade](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G2_ProjetoForum_Entrega01/commit/294e0e0569b690a8e64d9a47223528969740d5b0) | Elaboração do SIG/NFR complementar de Usabilidade, com delimitação, operacionalizações, trade-offs e relação aos achados ER-C01 a ER-C04. |
...
disponível no [arquivo BPMN 2.0 no GitHub](https://github.com/UnBArqDsw2026-2-Turma02/2026.2-T02-_G2_ProjetoForum_Entrega01/blob/a1ab8a0916676dab8baf87851e53f6ec47693c85/docs/Base/Relat%C3%B3rios/assets/Fluxo_Pergunta_Resposta_Resolucao.bpmn).

#### Visualização do modelo

![BPMN do fluxo de pergunta, resposta e resolução](./assets/Fluxo_Pergunta_Resposta_Resolucao.svg)
<p align="center"><b>Modelo BPMN do fluxo de pergunta, resposta e resolução</b><br>
<b>Autor:</b> Leonardo de Aquino</p>
...
| Leonardo de Aquino | Elaboração do SIG/NFR complementar de Usabilidade a partir dos insumos gerados pela Engenharia Reversa. | 28/08/2026 |
...
| `1.14` | 28/08/2026 | Inclusão do SIG/NFR complementar de Usabilidade, com diagrama, operacionalizações, trade-off e rastreabilidade aos achados ER-C01 a ER-C04, além dos ajustes correspondentes na metodologia do Foco_01 e na identificação de autoria dos SIGs. | [Leonardo de Aquino](https://github.com/surpesaiajin) | - | - |
| `1.15` | 28/08/2026 | Atualização da rastreabilidade dos artefatos complementares e registro da autoria da modelagem BPMN. | [Leonardo de Aquino](https://github.com/surpesaiajin) | - | - |
| `1.16` | 28/08/2026 | Correção dos URLs de rastreabilidade dos artefatos complementares e estabilização do permalink do arquivo-fonte BPMN. | [Leonardo de Aquino](https://github.com/surpesaiajin) | - | - |
