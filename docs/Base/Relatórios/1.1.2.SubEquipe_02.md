...
### SIG - NFR Framework
**1. A Escolha do Requisito Não-Funcional**

Com base na análise efetuada do Stack Overflow, o requisito escolhido foi  **Segurança contra abuso de reputação**. A ideia é simples: o fórum só funciona se a pontuação dos usuários (reputação) for confiável. Se for fácil burlar o sistema (criando contas falsas, usando bots ou trocando votos com amigos), a confiabilidade e engajamento das respostas perde valor.

**2. Diagrama SIG**

![Diagrama SIG - Segurança contra Abuso de Reputação](../Assets/DiagramaSIG.png)

**3. Rastreabilidade e Trade-offs**
*   **Objetivo Principal :** Segurança da Informação.
*   **Sub-objetivos:** Prevenção de Fraude de Votos e Prevenção de Manipulação em Grupo.
*   **Operacionalizações:**
    *   *Limite diário de votos:* Ajuda (`+ HELP`) a evitar fraudes, mas afeta negativamente (`- HURT`) a usabilidade de usuários engajados. 
    *   *Reputação mínima para votar:* Ajuda (`+ HELP`) a mitigar o uso de contas falsas.
    *   *Auditoria de votos:* Resolve ativamente (`++ MAKE`) o problema da manipulação em grupo (troca de votos mútua).
...
| Luis Guilherme Borges | Criação do diagrama SIG de acordo com o framework NFR | 25/08/2026 |
