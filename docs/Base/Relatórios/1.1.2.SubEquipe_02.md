...
#### Síntese do fluxo candidato ao BPMN

O fluxo possui escopo adequado para uma modelagem BPMN inicial por apresentar
atores distintos, sequência de atividades, decisões e caminhos alternativos:

`Autor identifica necessidade → publica pergunta → comunidade responde e avalia → autor acompanha → [aceita resposta / aguarda novas contribuições / pergunta é fechada] → resolução, espera ou possível reabertura`

Essa síntese ainda não é o BPMN final. Ela deverá ser revisada pela SubEquipe
02 e simplificada conforme o objetivo do projeto, evitando incorporar
mecanismos periféricos que não sejam necessários à experiência central do
fórum.
...
Ainda não definida. Será elaborada após a revisão da Engenharia Reversa
consolidada e a validação do fluxo pela SubEquipe_02.
